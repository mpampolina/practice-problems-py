''' My intepretation: if a cow value is already placed and the digit is exhausted, the game will not register a bull for
the further instances of the value.
example: Number is 1038. Guess is 1138. System will yield: 3 cows 0 bulls.

Also see: PracticePython-18-CowsAndBulls2.py for an (easier) alternate interpretation
'''
...
def CaB(guess_num):
    ...
    for count in range(len(guess)):
        if guess[count] == key[count]:
            cows += 1
            if guess[count] in mute:
                mute.remove(guess[count])
            else:
                bulls -= 1  # accounts for cases where a bull was registered before a cow
        elif guess[count] in mute:
            bulls += 1
            mute.remove(guess[count])
    return cows, bulls
